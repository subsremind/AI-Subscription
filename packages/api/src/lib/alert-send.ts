...
import { utcnow, dateToTZDate,dateToUTC, dateFormat, newDate } from "@repo/utils"
import {
	format,
	isSameDay,
	isSameHour,
...
export async function sendSubscriptionAlerts() {
	...
	for (const alert of alerts) {
        ...
        const triggerDate = newDate(triggerDateTZ.getFullYear(), triggerDateTZ.getMonth(), triggerDateTZ.getDate(), 9,0,0, alert.user?.timezone || "UTC");
		...
		// 	text: `Your subscription ${alert.subscriptionId} is due soon.`,
		// });
	}
}
